...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan.YXU','white'),color('BOT','red'),color('Whyuu','white'),color('AR18','yellow'))
})
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
               ...
               client.sendMessage(from, resul, text, { quoted: mek})
				...
				case 'mining':
                      ...
                      reply(`karena anda cogan anda mendapatkan mining sebesar ${one}Xp untuk anda`)
                      }else{
           ...
           case 'demote':
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ 𝙋𝙀𝙎𝘼?? 𝘽??𝙊𝘼𝘿𝘾𝘼𝙎?? ❯\n\n${body.slice(4)}`})
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 BOT YXU BROADCAST 」*\n\n${body.slice(4)}`)
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					}
					break
